...
```
╔══════════════════════════════════════════════════════════════════════════╗
║                         OpenCV.ts v5  (TypeScript)                       ║
╠══════════════════════════════════════════════════════════════════════════╣
║                                                                          ║
║  ┌─────────────────────── Applications ──────────────────────────────┐  ║
║  │                                                                    │  ║
║  │  JOSH Demo ✅              Realtime Demo ✅      Your App 🔲       │  ║
║  │  (offline, per-frame       (webcam, live          (import          │  ║
║  │   Adam 700 iters)           inference)             opencv-ts)      │  ║
║  │                                                                    │  ║
║  └────────────────────────────────────────────────────────────────────┘  ║
║                                 │                                        ║
║  ┌─────────────────────── Graph Engine ──────────────────────────────┐  ║
...
║  │   │              │              │                                  │  ║
║  │  Compiler      Executor      Scheduler                            │  ║
║  │  (DAG → plan)  (run plan)    (GPU/CPU dispatch)                   │  ║
║  └────────────────────────────────────────────────────────────────────┘  ║
║                                 │                                        ║
║  ┌─────────────────────── CV Kernels ─────────────────────────────────┐  ║
║  │                                                                    │  ║
║  │  Basic ops (src/kernels/)              JOSH ops (demo/josh/)       │  ║
║  │  ┌───────────────────────┐             ┌──────────────────────┐   │  ║
║  │  │ ✅ Color conversion   │             │ ✅ SMPL forward pass  │   │  ║
║  │  │ ✅ Gaussian blur      │             │ ✅ Rodrigues + FK     │   │  ║
║  │  │ ✅ Resize             │             │ ✅ Pose blend shapes  │   │  ║
║  │  │ ✅ Sobel edge detect  │             │ ✅ LBS skinning       │   │  ║
║  │  │ 🔲 Convolution (2D)   │             │ ✅ Adam optimizer     │   │  ║
║  │  │ 🔲 Optical flow       │             │ ✅ 6 loss kernels     │   │  ║
║  │  │ 🔲 Feature detection  │             │ ✅ JVP chain rule     │   │  ║
║  │  │ 🔲 Homography         │             │ ✅ 3D renderer        │   │  ║
║  │  │ 🔲 Stereo matching    │             └──────────────────────┘   │  ║
║  │  └───────────────────────┘                                         │  ║
║  │                                                                    │  ║
║  │  Models (ONNX via ort-web)                                         │  ║
║  │  ✅ MiDaS depth   ✅ MoveNet 2D   ✅ BlazePose 3D                  │  ║
║  │  🔲 MASt3R stereo  🔲 SAM segmentation  🔲 DepthAnything v2        │  ║
║  └────────────────────────────────────────────────────────────────────┘  ║
║                                 │                                        ║
║  ┌─────────────────────── Compute Backends ───────────────────────────┐  ║
║  │                                                                    │  ║
║  │   WebGPU ✅              WASM (Rust) ✅        CPU (JS) ✅          │  ║
║  │   src/backends/webgpu/   src/backends/wasm/    fallback            │  ║
║  │                                                                    │  ║
║  │   WGSL shaders (25)      Contact detection     Buffer ops          │  ║
║  │   Pipeline cache         L-BFGS solver         Tensor math         │  ║
║  │   Texture binding        wasm-pack build                           │  ║
║  │                                                                    │  ║
║  │   🔲 WebNN backend        🔲 WASM SIMD ops      🔲 Worker threads   │  ║
║  └────────────────────────────────────────────────────────────────────┘  ║
║                                 │                                        ║
║  ┌─────────────────────── Core Types ─────────────────────────────────┐  ║
║  │  src/core/  ✅                                                     │  ║
║  │                                                                    │  ║
║  │  Mat / Tensor   BufferManager   ResourceTracker   Dtype system     │  ║
║  └────────────────────────────────────────────────────────────────────┘  ║
║                                                                          ║
╠══════════════════════════════════════════════════════════════════════════╣
║  ✅ Implemented   🔲 Planned                                              ║
...
### JOSH pipeline (the stress-test demo)
...
```
 ...
 │ Frame extrac│    5 FPS sample rate                        ✅
 └──────┬──────┘
        │ frames[]
        ▼
 ┌─────────────┐    tf.js BodyPix / HSV threshold
 │ Segmentation│    → binary foreground mask                 ✅
 └──────┬──────┘
        │ masks[]
        ▼
 ┌─────────────┐    MiDaS v2.1-small (ONNX, WebGPU EP)
 │ Depth estim.│    → relative depth map [H×W]               ✅
 └──────┬──────┘
        │
        ├──────────────────────────────────┐
        ▼                                  ▼
 ┌─────────────┐                    ┌─────────────┐
 │ 3D unproject│                    │ Focal recov.│  least-squares f from
 │ (MASt3R API)│                    │ (WASM)      │  depth+pixel coords   ✅
 └──────┬──────┘                    └──────┬──────┘
        │ pts3d [H×W×3]                    │ K (intrinsics)
        ▼                                  │
 ┌─────────────┐    BlazePose 3D (tf.js)   │
 │ Pose init   │    → 24 SMPL joint angles  │                 ✅
 │ (ROMP API)  │       10 shape betas       │
 └──────┬──────┘                           │
        │ smplInit                         │
        ▼                                  │
 ┌─────────────┐    MoveNet Lightning ONNX  │
 │ 2D keypoints│    → 17 COCO joints + conf │                 ✅
 └──────┬──────┘                           │
        │ kp2d                             │
        ▼                                  │
 ┌─────────────┐    geometric foot test     │
 │ Contact det.│    → per-vertex mask       │                 ✅
 │ (WASM)      │                            │
 └──────┬──────┘                           │
        │ contactMask                      │
        └──────────────┬───────────────────┘
                       ...
                       ▼
              ┌─────────────────┐
              │  JOSH Optimizer │   tf.variableGrads + tf.train.adam
              │  Stage 1        │   500 iters, lr=0.07                ✅
              │  L_3D + L_c1 +  │
              │  L_c2 + L_p +   │
              │  L_smooth       │
              │                 │
              │  Stage 2        │   200 iters, lr=0.014               ✅
              │  + L_2D         │
              └────────┬────────┘
                       │ pose[72], betas[10], transl[3]
                       ▼
              ┌─────────────────┐
              │  SMPL mesh      │   6890 vertices, 24 joints           ✅
              │  WebGPU render  │   Phong shading + point cloud
              └─────────────────┘
```
